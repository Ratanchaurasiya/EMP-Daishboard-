...
// Support MySQL connection URL or standard environment variables
const DATABASE_URL = process.env.DATABASE_URL || process.env.MYSQL_URL || null;
...
// Initialize MySQL / MariaDB Connection Pool if DATABASE_URL or MYSQL_URL is supplied
if (DATABASE_URL && (DATABASE_URL.startsWith('mysql://') || DATABASE_URL.startsWith('mysql2://') || DATABASE_URL.includes('mysql') || process.env.MYSQL_HOST)) {
  try {
    ...
    // Test connection synchronously at startup
    const connection = await mysqlPool.getConnection();
    activeEngine = 'MySQL / MariaDB';
  ...
  throw new Error('DATABASE_URL environment variable is required in production mode for MySQL persistence.');
}

// If MySQL is not active (in local development only), initialize SQLite
