...
export const EmployeeDashboard: React.FC<EmployeeDashboardProps> = () => {
  ...
  // Auto-open Service Receipt Modal when notification item is clicked
  useEffect(() => {
    if (highlightedServiceId) {
      setActiveSection('maintenance');
      const foundRecord = serviceRecords.find(s => s.id === highlightedServiceId);
      if (foundRecord) {
        setPreviewReceiptRecord(foundRecord);
      }
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-5">
          ...
          <div className="flex items-center gap-2.5 w-full md:w-auto shrink-0">
            ...
            <button
              type="button"
              onClick={logout}
              className="inline-flex items-center justify-center gap-1.5 px-3.5 py-2 text-xs font-bold text-rose-200 bg-rose-500/20 hover:bg-rose-500/30 rounded-xl border border-rose-500/30 transition-colors cursor-pointer"
                  ...
                  <div className="mt-1 flex items-center gap-1.5">
                    <span
                      className={`px-2 py-0.5 rounded text-[11px] font-bold ${
                        assignedComputer.securityFunctionAdded === 'No'
                          ? 'bg-rose-500/10 text-rose-600 dark:text-rose-400 border border-rose-500/20'
                          : 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20'
                      }`}
                    >
                      {assignedComputer.securityFunctionAdded === 'No' ? 'Not Added' : 'Added'}
                    </span>
                  </div>
                  <div className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5 font-mono">
                    Date: {assignedComputer.securityFunctionAdded === 'No' ? '—' : formatDateDisplay(assignedComputer.securityFunctionAddedDate || assignedComputer.assignedDate)}
                  </div>
                </div>
                          ...
                          <td className="py-3.5 px-3.5 text-slate-800 dark:text-slate-200 font-semibold">
                            ...
                            {(asset.assetType === 'Laptop' || asset.assetType === 'Desktop' || asset.assetType === 'Mobile Phone') && (
                              <div className="mt-1 flex items-center gap-1.5 flex-wrap">
                                ...
                                >
                                  🛡️ Security Status: {asset.securityFunctionAdded === 'No' ? 'Not Added' : 'Added'}
                                </span>
                                <span className="text-[10px] text-slate-500 dark:text-slate-400 font-mono">
                                  Date: {asset.securityFunctionAdded === 'No' ? '—' : formatDateDisplay(asset.securityFunctionAddedDate || asset.assignedDate)}
                                </span>
                              </div>
                            )}
                          </td>
      ...
      {(activeSection === 'all' || activeSection === 'updates') && (
        <div className="bg-white dark:bg-[#101726] rounded-2xl border border-slate-200 dark:border-slate-800 p-5 sm:p-6 shadow-xs">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-4">
            ...
            {/* Dynamic Service & Repair Receipt Notification Card */}
            {(() => {
              const latestServiceWithReceipt = myServiceRecords.find(s => s.receiptFileUrl || (s.serviceCost && Number(s.serviceCost) > 0));
              if (!latestServiceWithReceipt) return null;

              return (
                <div className="p-3.5 bg-blue-50/60 dark:bg-blue-950/30 rounded-xl border border-blue-200/80 dark:border-blue-900/50 flex items-start gap-3 col-span-1 md:col-span-2 shadow-2xs">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between">
                      <h4 className="text-xs font-bold text-slate-900 dark:text-white flex items-center gap-1.5">
                        <span>🔧 New Service &amp; Repair Receipt Added</span>
                      </h4>
                      <button
                        type="button"
                        onClick={() => setPreviewReceiptRecord(latestServiceWithReceipt)}
                        className="text-[10px] font-bold text-blue-600 dark:text-blue-400 bg-blue-100 dark:bg-blue-900/60 hover:bg-blue-200 dark:hover:bg-blue-800 px-2 py-0.5 rounded cursor-pointer transition-colors"
                      >
                        View Receipt
                      </button>
                    </div>
                    <p className="text-[11px] text-slate-600 dark:text-slate-400 mt-1 font-medium">
                      Admin has added a new service/repair receipt for your {latestServiceWithReceipt.deviceName || latestServiceWithReceipt.assetNumber || 'Laptop/Asset'}. Click to view details.
                    </p>
                    <div className="text-[10px] text-slate-400 mt-1.5 flex items-center gap-2 flex-wrap">
                      <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> Service Date: {latestServiceWithReceipt.serviceDate || 'N/A'}</span>
                      {latestServiceWithReceipt.serviceProviderShopName && (
                        <span>• Provider: {latestServiceWithReceipt.serviceProviderShopName}</span>
                      )}
                      {latestServiceWithReceipt.serviceCost ? (
                        <>
                          <span>•</span>
                          <span className="font-mono font-bold text-blue-600 dark:text-blue-400">
                            Amount: ₹{Number(latestServiceWithReceipt.serviceCost).toLocaleString('en-IN')}
                          </span>
                        </>
                      ) : null}
                    </div>
