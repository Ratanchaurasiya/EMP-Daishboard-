...
export const LoginScreen: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      {/* =================================================================== */}
      {/* 1. BACKGROUND IMAGE: WOMAN WITH RAISED FINGER & SKYSCRAPERS          */}
      {/* Kept 100% clearly visible, zero dark gradient covering the woman    */}
      {/* =================================================================== */}
      <div className="absolute inset-0 z-0 pointer-events-none select-none overflow-hidden">
        <img
          src="/login-hero.png"
          alt="AssetCore Enterprise Identity Landscape"
          onLoad={updateFingertipCoordinates}
          className="absolute max-w-none pointer-events-none select-none transition-all duration-300 ease-out"
          style={{
            width: `${heroImgLayout.width}px`,
            height: `${heroImgLayout.height}px`,
            left: `${heroImgLayout.left}px`,
            top: `${heroImgLayout.top}px`,
          }}
        />

        {/* Soft, targeted gradient behind the login card on the right */}
        <div className="absolute inset-y-0 right-0 w-full md:w-[46%] lg:w-[38%] bg-gradient-to-l from-[#05070a]/35 via-[#05070a]/15 to-transparent z-10" />

        {/* Delicate soft left vignette to blend edge */}
        <div className="absolute inset-y-0 left-0 w-24 bg-gradient-to-r from-[#05070a]/60 to-transparent z-10" />

        {/* Delicate Top & Bottom Vignettes (preserving the woman's face & finger) */}
        <div className="absolute top-0 inset-x-0 h-24 bg-gradient-to-b from-[#05070a]/80 to-transparent z-10" />
        <div className="absolute bottom-0 inset-x-0 h-24 bg-gradient-to-t from-[#05070a]/85 to-transparent z-10" />
      </div>

      {/* =================================================================== */}
      {/* 2. INTERACTIVE GOLDEN ENERGY CONNECTION LAYER (SVG & Canvas)        */}
      {/* =================================================================== */}
      {fingerPos && (
        <div className="absolute inset-0 pointer-events-none z-15 overflow-hidden">
          {/* SVG Vector Energy Trail from Fingertip to Card */}
          {cardTargetPos && bezierTrailPath && (
            <svg
              className="absolute inset-0 w-full h-full overflow-visible"
              xmlns="http://www.w3.org/2000/svg"
            >
              <defs>
                {/* Glow Filter for High-Impact Luxury Luster */}
                ...
                {/* Golden Linear Gradient along the Path */}
                <linearGradient id="goldEnergyGrad" x1="100%" y1="0%" x2="0%" y2="100%">
                  <stop offset="0%" stopColor="#fef08a" stopOpacity="1" />
                  <stop offset="35%" stopColor="#f59e0b" stopOpacity="0.9" />
                  <stop offset="75%" stopColor="#d97706" stopOpacity="0.75" />
                  <stop offset="100%" stopColor="#fbbf24" stopOpacity="0.95" />
                </linearGradient>

                <linearGradient id="goldCoreGrad" x1="100%" y1="0%" x2="0%" y2="100%">
              ...
              <path
                ...
                strokeWidth={2.5 + cursorProximity * 1.5}
                filter="url(#trailGlow)"
                className="transition-all duration-700"
                style={{
                  opacity: entranceStage === 'ignite' ? 0 : 0.85,
                  strokeDasharray: '400',
                  strokeDashoffset: entranceStage === 'ignite' ? '400' : '0',
                  transition: 'stroke-dashoffset 0.8s cubic-bezier(0.16, 1, 0.3, 1), opacity 0.5s ease',
                }}
              />

              {/* Ultra-Crisp Core Laser Line */}
              <path
                d={bezierTrailPath}
                fill="none"
                ...
                strokeWidth={1.2}
                style={{
                  opacity: entranceStage === 'ignite' ? 0 : 0.95,
                  strokeDasharray: '400',
                  strokeDashoffset: entranceStage === 'ignite' ? '400' : '0',
                  transition: 'stroke-dashoffset 0.8s cubic-bezier(0.16, 1, 0.3, 1), opacity 0.5s ease',
                }}
              ...
              {/* Animated Traveling Light Pulses along the Trail */}
              {entranceStage !== 'ignite' && (
                <>
                  <path
                    ...
                    style={{
                      animation: 'goldenLaserPulse 3.5s linear infinite',
                  ...
                  <path
                    ...
                    style={{
                      animation: 'goldenLaserPulse 2.4s linear infinite',
          ...
          {/* ------------------------------------------------------------- */}
          {/* FINGERTIP EMISSION NODE: Exact Pixel Placement on Her Finger */}
          {/* ------------------------------------------------------------- */}
          <div
          ...
          >
            {/* Concentric Golden Sonar Waves / Ripples */}
            <div
              className="absolute -inset-5 rounded-full border border-amber-400/60"
              style={{ animation: 'fingertipSonar 2.8s cubic-bezier(0.2, 0.8, 0.2, 1) infinite' }}
            />
            <div
              className="absolute -inset-5 rounded-full border border-yellow-300/40"
              style={{
                animation: 'fingertipSonar 2.8s cubic-bezier(0.2, 0.8, 0.2, 1) infinite',
                animationDelay: '0.9s',
              }}
            />
            <div
              className="absolute -inset-5 rounded-full border border-amber-500/30"
              style={{
                animation: 'fingertipSonar 2.8s cubic-bezier(0.2, 0.8, 0.2, 1) infinite',
                animationDelay: '1.8s',
              }}
            ...
            <div
              className="absolute -inset-8 rounded-full bg-gradient-to-r from-amber-400/30 via-yellow-300/20 to-amber-500/30 blur-md pointer-events-none transition-transform duration-300"
              style={{
                transform: `scale(${1 + cursorProximity * 0.8})`,
              }}
            />

            {/* Glowing Golden Pinpoint Orb at Exact Fingertip */}
            <div
              className="relative w-4 h-4 rounded-full bg-gradient-to-tr from-amber-500 via-yellow-200 to-white shadow-[0_0_14px_#fbbf24] flex items-center justify-center transition-transform duration-300"
              style={{
                transform: `scale(${1 + cursorProximity * 0.4})`,
                animation: 'energyOrbPulse 2.2s ease-in-out infinite',
              }}
            >
            ...
            {/* Subtle 4-Point Golden Star Flare */}
            <div
              className="absolute -inset-2 opacity-80 pointer-events-none"
              style={{ animation: 'starRotate 16s linear infinite' }}
            >
              <div className="w-full h-full relative flex items-center justify-center">
                <div className="absolute w-8 h-[1.5px] bg-gradient-to-r from-transparent via-yellow-200 to-transparent" />
                <div className="absolute h-8 w-[1.5px] bg-gradient-to-b from-transparent via-yellow-200 to-transparent" />
              </div>
            </div>

            {/* Subtle Interactive Telemetry Tag */}
            {/* Subtle Interactive Telemetry Tag placed neatly below the fingertip */}
            <div
              className="absolute left-1/2 -translate-x-1/2 top-7 whitespace-nowrap px-2 py-0.5 rounded-full bg-black/75 backdrop-blur-md border border-amber-400/40 text-[9px] font-mono text-amber-300 font-bold shadow-lg transition-opacity duration-300"
              style={{
                opacity: 0.75 + cursorProximity * 0.25,
              }}
            >
              <span className="inline-block w-1.5 h-1.5 rounded-full bg-amber-400 animate-ping mr-1" />
              BIOMETRIC ORIGIN • ACTIVATED
            </div>
          </div>

          {/* ------------------------------------------------------------- */}
          {/* CARD CONNECTION NODE: Point of Impact on the Login Panel     */}
          {/* ------------------------------------------------------------- */}
          {cardTargetPos && entranceStage !== 'ignite' && (
            <div
              className="absolute transform -translate-x-1/2 -translate-y-1/2 pointer-events-none"
              style={{
                left: `${cardTargetPos.x}px`,
                top: `${cardTargetPos.y}px`,
              }}
            >
              <div className="relative w-3 h-3 rounded-full bg-amber-400 shadow-[0_0_12px_#f59e0b] flex items-center justify-center animate-pulse">
